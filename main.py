...
while poker.get_winner() is None:
    ...
    if current_type is not None and current_player != current_type_player:
        ...
        is_pass = input("Do you want to play cards against current?(Yes, No):").lower()
        if is_pass == "no":
    ...
    if current_type is not None:
        if not card_type > current_type:
            is_greater = False

    while not is_greater or card_type.name == CardType.CARD_TYPE_INVALID_TYPE or not Common.is_contain_sub_list(current_player.hand_cards, cards):
